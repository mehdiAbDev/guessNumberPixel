...
const checkTheNumber = function() {
    ...
    if (!theGuess) {
        ...
        document.body.style.backgroundColor = '#111';
            score--;
            theScore.textContent = score;
            displayMessage(theScore, score);
            displayMessage(theMessage,
                theGuess > theSecretNumber
                    ? `🔽Go down`
                    : `🔼 Go up`
            );
        } else {
...
document.addEventListener('keydown', function(event){
    if(event.key === '' || event.key === 'r' ) resetGame() ;
    else if (event.key === ' ' ||  event.key === 'Enter') checkTheNumber();
